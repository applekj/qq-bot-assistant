...
def bazi_cesuan(query: str):
    ...
    prompt = prompt.partial(format_instructions=parser.get_format_instructions(), api_key=settings.yuanfenju_api_key)
    chain = prompt | ChatOpenAI(model="qwen-max",
            temperature=0,
            streaming=True,
            api_key=settings.openai_api_key,
            base_url=settings.openai_api_base,) | parser
    ...
    if not data.get('hours'):
        return "请提供出生小时"
    if data.get('minute') is None:
        data['minute'] = ''  # 为分钟提供默认值
...
def jiemeng(query: str):
    ...
    需要：用户描述梦境内容
    """
    api_key = settings.yuanfenju_api_key
